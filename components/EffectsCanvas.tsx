...
import type { Projectile, EnemyProjectile, ShellCasing, Gib, DamageNumber, Explosion, SplatterParticle } from '../types';
...
const EffectsCanvas: React.FC<EffectsCanvasProps> = (props) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      const { projectiles, enemyProjectiles, shellCasings, gibs, damageNumbers, explosions, projectileColor, reviveTriggerTime, width, height } = latestProps.current;
      ...
      if (reviveTriggerTime > 0 && now - reviveTriggerTime < revivePulseDuration) {
          ...
          // An ease-out quint function to approximate the original cubic-bezier(0.25, 1, 0.5, 1)
          const easeOutQuint = (t: number) => 1 - Math.pow(1 - t, 5);
          const easedProgress = easeOutQuint(progress);

          const scale = easedProgress * 2;
          const opacity = 0.8 * (1 - progress); // The opacity is linear in the original animation

          if (scale > 0 && opacity > 0) {
              const centerX = width / 2;
              const centerY = height / 2;
              // The final radius is `width` because the element is `w-full` and scales to `2`.
              const radius = (width / 2) * scale; 

              const gradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, radius);
              
              // The alpha of the color stops is modulated by the overall opacity of the pulse
      ...
      // --- Draw Splatter Explosions (Optimized) ---
      const splatterColors = ['#f09', '#ff007f', '#fff'];
      ctx.shadowBlur = 10; // Common shadow blur for all splatter particles
      ...
      explosions.forEach(exp => {
        ...
        // Use eased progress for opacity and scale to match CSS `ease-out` for visual fidelity
        ...
        const particlesByColor: Record<string, SplatterParticle[]> = {};
        splatterColors.forEach(c => { particlesByColor[c] = []; });
        ...
        for (const color of splatterColors) {
          ...
          // Set color properties once per color group
          ctx.fillStyle = color;
          ...
          particles.forEach(p => {
            ...
            if (currentRadius > 0) {
              // Using moveTo before arc for disconnected shapes in one path is a good practice
      ...
      // Reset context properties that were changed during explosion rendering
      ctx.globalAlpha = 1;
      ctx.shadowBlur = 0;
      ...
      projectiles.forEach(p => {
        ...
        // 1. Outer Glow (the main color of the projectile)
        ...
        // 2. Inner Core (the white-hot center)
        ctx.shadowBlur = 0; // No shadow for the core to make it sharp
      ...
      ctx.shadowColor = '#22c55e'; // emerald-500
      ...
      damageNumbers.forEach(dn => {
        ...
        if (dn.isCrit) {
            // Replicate 'crit-float-up-fade-out' keyframes
            if (progress < 0.2) {
            ...
            // Replicate 'float-up-fade-out' keyframes
            yOffset = -40 * progress;
            opacity = 1 - progress;
        }
        ...
        if (dn.isCrit) {
            ...
            // Draw yellow glow shadow
            ctx.shadowColor = '#ff0';
            ctx.shadowBlur = 5;
            ctx.fillStyle = '#fde047'; // yellow-300
            ctx.fillText(dn.text, 0, 0);

            // Draw black text shadow on top
            ctx.shadowColor = '#000';
            ctx.shadowBlur = 2;
            ctx.fillText(dn.text, 0, 0);
        } else {
